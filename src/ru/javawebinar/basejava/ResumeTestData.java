...
public class ResumeTestData {
    ...
    public static void main(String[] args) {
        ...
        Company.Period alcatelPeriod = new Company.Period(9, 1997, 1, 2005, "Инженер по аппаратному и программному тестированию", "Тестирование, отладка, внедрение ПО цифровой телефонной станции Alcatel 1000 S12 (CHILL, ASM).");
        Company.Period siemensPeriod = new Company.Period(1, 2005, 2, 2007, "Разработчик ПО", "Разработка информационной модели, проектирование интерфейсов, реализация и отладка ПО на мобильной IN платформе Siemens @vantage (Java, Unix).");
        Company.Period enkataPeriod = new Company.Period(3, 2007, 6, 2008, "Разработчик ПО", "Реализация клиентской (Eclipse RCP) и серверной (JBoss 4.2, Hibernate 3.0, Tomcat, JMS) частей кластерного J2EE приложения (OLAP, Data mining).");
        Company.Period yotaPeriod = new Company.Period(6, 2008, 12, 2010, "Ведущий специалист", "Дизайн и имплементация Java EE фреймворка для отдела \"Платежные Системы\" (GlassFish v2.1, v3, OC4J, EJB3, JAX-WS RI 2.1, Servlet 2.4, JSP, JMX, JMS, Maven2). Реализация администрирования, статистики и мониторинга фреймворка. Разработка online JMX клиента (Python/ Jython, Django, ExtJS)");
        Company.Period luxoftPeriod = new Company.Period(12, 2010, 4, 2012, "Ведущий программист", "Участие в проекте Deutsche Bank CRM (WebLogic, Hibernate, Spring, Spring MVC, SmartGWT, GWT, Jasper, Oracle). Реализация клиентской и серверной части CRM. Реализация RIA-приложения для администрирования, мониторинга и анализа результатов в области алгоритмического трейдинга. JPA, Spring, Spring-MVC, GWT, ExtGWT (GXT), Highstock, Commet, HTML5.");
        Company.Period ritCenterPeriod = new Company.Period(4, 2012, 10, 2014, "Java архитектор", "Организация процесса разработки системы ERP для разных окружений: релизная политика, версионирование, ведение CI (Jenkins), миграция базы (кастомизация Flyway), конфигурирование системы (pgBoucer, Nginx), AAA via SSO. Архитектура БД и серверной части системы. Разработка интергационных сервисов: CMIS, BPMN2, 1C (WebServices), сервисов общего назначения (почта, экспорт в pdf, doc, html). Интеграция Alfresco JLAN для online редактирование из браузера документов MS Office. Maven + plugin development, Ant, Apache Commons, Spring security, Spring MVC, Tomcat,WSO2, xcmis, OpenCmis, Bonita, Python scripting, Unix shell remote scripting via ssh tunnels, PL/Python");
        Company.Period wrikePeriod = new Company.Period(10, 2014, 1, 2016, "Старший разработчик (backend)", "Проектирование и разработка онлайн платформы управления проектами Wrike (Java 8 API, Maven, Spring, MyBatis, Guava, Vaadin, PostgreSQL, Redis). Двухфакторная аутентификация, авторизация по OAuth1, OAuth2, JWT SSO.");
        Company.Period javaOnlineProjectsPeriod = new Company.Period(10, 2013, "Автор проекта.", "Создание, организация и проведение Java онлайн проектов и стажировок.");
        ...
        Company.Period mftiTeachingPeriod = new Company.Period(9, 1984, 6, 1987, "Закончил с отличием", null);
        Company.Period spbNIEITMiOTeachingPeriod1 = new Company.Period(9, 1987, 7, 1993, "Инженер (программист Fortran, C)", null);
        Company.Period spbNIEITMiOTeachingPeriod2 = new Company.Period(9, 1993, 7, 1996, "Аспирантура (программист С, С++)", null);
        Company.Period alcatelTeachingPeriod = new Company.Period(9, 1997, 3, 1998, "6 месяцев обучения цифровым телефонным сетям (Москва)", null);
        Company.Period siemensTeachingPeriod = new Company.Period(1, 2005, 4, 2005, "3 месяца обучения мобильным IN сетям (Берлин)", null);
        Company.Period luxoftTeachingPeriod = new Company.Period(3, 2011, 4, 2011, "Курс \"Объектно-ориентированный анализ ИС. Концептуальное моделирование на UML.\"", null);
        Company.Period courseraTeachingPeriod = new Company.Period(3, 2013, 5, 2013, "\"Functional Programming Principles in Scala\" by Martin Odersky", null);
    ...
    public static Resume createResume(String uuid, String fullName) {
        ...
        resume.addSection(SectionType.EXPERIENCE, new CompaniesSection(
                new Company("http://www." + randomString(random.nextInt(9), ALPHABET) + ".ru/", randomString(random.nextInt(9), ALPHABET),
                        new Company.Period(random.nextInt(11) + 1, 1980 + random.nextInt(39), random.nextInt(11)+1, 1980 + random.nextInt(39), randomString(random.nextInt(15), ALPHABET_RUS), randomString(random.nextInt(99), ALPHABET_RUS))
                ),
                new Company("http://www." + randomString(random.nextInt(9), ALPHABET) + ".ru/", randomString(random.nextInt(9), ALPHABET),
                        new Company.Period(random.nextInt(11) + 1, 1980 + random.nextInt(39), random.nextInt(11)+1, 1980 + random.nextInt(39), randomString(random.nextInt(15), ALPHABET_RUS), randomString(random.nextInt(99), ALPHABET_RUS))
                )
